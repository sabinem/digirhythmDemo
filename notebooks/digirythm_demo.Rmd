...
filename <- file.path('../data/', '516b_2.csv')
...
We can then remove outlier observations usig `remove_activity_outliers`. This function works on each column separately and replace outlier values by the mean value (computed without outliers):

```{r}
print(paste0("max value before outliers were removed: ", max(df$Motion.Index)))
df_cleaned <- remove_activity_outliers(df)
print(paste0("max value after the removal of outliers: ", max(df_cleaned$Motion.Index)))
```

After removing outliers, the maximum motion value went from `r max(data$Motion.Index)` to `r max(df.no.outliers$Motion.Index)`.
...
We can also resample the data to a lower periodicity, which will combine samples falling into the same period. Below, we resample the data from `r dgm_periodicity(data)$frequency` to 30 minutes.

```{r}
df_30 <- resample_dgm(df, 30)
dgm_periodicity(df_30)
```
...
The Degree of Functional Coupling (DFC) represents the proportion of cyclic activity that is harmonically synchronized with environmental rythms, over a 24-hour period. This is often used to measure synchrony between the circadian rythm and the environment. A DFC of 1 means 100% of the cyclic activity is fully synchronized with the environment, whereas a value of 0 means they are independent.

DigiRhythm can compute the DFC over a 7-day rolling window and plot it using the `dfc` function.
